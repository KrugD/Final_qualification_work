...
1. **Диаризация (Diarization)** — определение говорящих и временных меток их речи (с мерджем соседних сегментов)
2. **Распознавание речи (ASR)** — транскрибация аудио в текст
3. **Коррекция текста (Correction)** — исправление ошибок ASR до суммаризации
...
│   ├── diarization.py            # Модуль диаризации спикеров (с мерджем сегментов)
│   ├── asr.py                    # Модуль распознавания речи
│   ├── correction.py             # Модуль коррекции текста (исправление ошибок ASR)
...
| `*_correction.txt` | Исправленные результаты ASR (коррекция ошибок распознавания) |
...
# Максимальная длина текста для суммаризации (в токенах)
MAX_SUMMARY_INPUT_TOKENS = 512  # токены
