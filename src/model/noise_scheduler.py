...
class AdaptiveNoiseScheduler(NoiseScheduler):
    """
    Adaptive noise scheduler that adjusts masking based on token confidence.
    
    Inspired by the sufficiency-guided masking from Arg-LLaDA.
    """
    ...
    def __init__(
        ...
        remasking_ratio: float = 0.3,
    ):
        super().__init__(num_diffusion_steps, mask_token_id, schedule_type)
        self.confidence_threshold = confidence_threshold
        self.remasking_ratio = remasking_ratio
    
    def confidence_guided_remasking(
        self,
        token_ids: torch.Tensor,
        ...
        """
        Remask tokens with low confidence for refinement.
        
        Args:
            token_ids: Current token predictions [batch_size, seq_len]
            token_probs: Confidence probabilities [batch_size, seq_len]
            attention_mask: Optional attention mask
        
        Returns:
            remasked_tokens: Tokens with low-confidence positions masked
            remask_positions: Boolean mask of remasked positions
        """
        batch_size, seq_len = token_ids.shape
        device = token_ids.device
        
        # Find low confidence tokens
        low_confidence = token_probs < self.confidence_threshold
        
        # Apply attention mask
        if attention_mask is not None:
            low_confidence = low_confidence & attention_mask.bool()
        
        # Randomly select a subset of low confidence tokens to remask
        rand_select = torch.rand(batch_size, seq_len, device=device)
        remask_positions = low_confidence & (rand_select < self.remasking_ratio)
        
        # Apply remasking
        remasked_tokens = token_ids.clone()
        remasked_tokens[remask_positions] = self.mask_token_id
        
        return remasked_tokens, remask_positions
